...
`neteasePlaylistId` 填网易云歌单页面 URL 中的歌单 ID，不需要手写歌曲名和作者。当前使用网易云官方 `type=0` 外链播放器，高度为 450px，宽度会随页面自适应。官方播放器只向 iframe 输出前 10 首歌曲，不会把第 11 首及之后的条目交给本站，因此本站不能为后续歌曲补上滚动列表；如需完整歌单，需要改用可读取完整歌单数据的播放器方案。
...
`fun` 支持多个折叠分组，每组可以配置标题、说明和任意数量的图片。图片可以放在 `public/` 下，再从网站根路径引用。该区块横跨 ABOUT 内容区全宽，图片不预裁切并保持原始宽高比；点击图片会打开灯箱放大查看。
...
桌面端的介绍、CONTACT 链接、播放器和 OTHER 等双栏区块共用同一条起始轴；左侧小标题与头像左边缘对齐。SOMETHING FUN 是例外，会从左侧标题轴延伸到内容区最右侧。
