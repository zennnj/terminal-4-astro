...
- Memos are local Markdown entries displayed as circular-avatar-led white speech bubbles. On desktop, the page keeps its natural vertical scroll while the configurable square-corner sticky notes use sticky positioning in the left rail; the feed must not create its own bordered or clipped scroll panel. Mobile stacks the notes above the natural-flow feed. Memo frontmatter has no title or tags and may expose a short footnote at the bubble's lower left. Each bubble links to its Waline-backed detail comments, while Junk remains a standalone Waline thread.
...
- Gallery artwork opens in a moderate modal capped at `1260px × 760px`, reserving enough horizontal viewport space for external navigation controls. Its description remains a narrow `280px–360px` P4-gray rail while the image consumes the remaining space. The rounded, shadowed inner layout places the image on a continuous P4-purple base so antialiased image-side corners cannot expose a light parent seam; the image frame has no additional padding or placeholder color. Circular previous/next controls sit completely outside the modal's left and right borders, wrap through only the currently filtered artworks, and mirror the keyboard Left/Right Arrow behavior.
...
- The REVIEW directory uses a compact yellow latest-review feature card. Its orange `Latest` burst intentionally overlaps outside the card's upper-left edge, while the Noto Sans SC `Latest` text must remain fully contained inside the orange burst. The description-to-card gap is compact, while the card-to-filter gap is deliberately more generous. Filters have yellow capsule hover/focus feedback. Directory entries are content-width, wrapping capsules in the ABOUT ME favorites language, ordered as a small category bullet, title, muted parenthesized creator, and date. Pagination is based on the rendered wrap layout at 10 visual rows per page and must be recalculated after filtering, font readiness, or container-width changes.
