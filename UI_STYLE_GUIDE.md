...
- Gallery directory cards use a row-major two-column desktop grid and a single mobile column. Preserve date-descending DOM order as the visible left-to-right, top-to-bottom order; do not use CSS multi-column flow, which fills vertically before moving to the next column. Directory thumbnails use a fixed `4 / 3` cover crop and fixed-height captions so strict ordering does not create uneven row gaps; the modal remains the uncropped full-art view.
