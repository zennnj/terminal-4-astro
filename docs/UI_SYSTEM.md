...
- Memo directory entries are local Markdown rendered as compact, circular-avatar-led speech bubbles. On desktop the page retains its natural vertical scroll and the square-corner notes from `src/config/memos.ts` stay in view through sticky positioning; the feed does not own a nested scroll container. Mobile uses natural page flow. Memo frontmatter omits title and tags, with an optional lower-left footnote. Per-entry comments remain on the Waline-backed detail route and Junk remains a standalone Waline thread.
