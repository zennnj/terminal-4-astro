...
- Detail pages: Article, Memo, and Review details. Use `BackLink`, the detail title scale, readable content width, and optional TOC/media regions. Article details place the full left column in one `92%` white, shared-radius panel with the Article directory card shadow while keeping the TOC separate. Review details reuse that TOC beside the Markdown REVIEW body only when the body contains level 1–4 headings; without headings they retain the existing single-column body and render no empty side region.
...
- Gallery modal media is the exception to intrinsic directory sizing: the dialog fills the viewport, with a translucent black image canvas on the left and a full-height, independently scrolling `300px–350px` P4-gray information rail on the right. The image uses `object-fit: contain` against the complete canvas with no authored inset, so portrait art can touch the top and bottom edges and landscape art can touch the side edges; only aspect-ratio letterboxing remains. White-on-black translucent close, zoom, and previous/next controls remain over the image canvas; zoom ranges from `1×` to `3×` and resets when the artwork changes or closes. Edge buttons and the Left/Right Arrow keys cycle through the current filtered artwork set and wrap at both ends. On narrow phones, the same regions stack vertically so neither the image nor its metadata is squeezed into an unusable column.
...
Article, Memo, Review, and Gallery families use only the shared quiet white `28px` dot matrix over P4 gray. Do not combine it with light pools or paper grain. Review's supplied ticket, rotating polygon layers, and random label are explicit local artwork layers over that single page surface; they do not change the standard Review shell or PageHeader. Review currently mounts no bottom-right mascot.

The unmounted `ReviewMascot.astro` component and its source frames are retained for migration to a different page. It follows the Homepage sequence convention: its nine exported files contain three repeated copies of each pose, so only frame numbers `0000`, `0003`, and `0006` play at `100ms` per pose. Its sixteen-loop phase order remains eight person-only loops, four `Game is life` loops, then four `Don't you know?` loops.

Review ticket overlays render Chinese copy as Taiwan Traditional Chinese through a server/build-time converter; authored content remains untouched. A title over 10 weighted visual units splits at its middle character into two centered lines at half the normal responsive title size. The ticket center uses a plain-text excerpt of the Markdown body's first non-empty paragraph rather than the frontmatter `summary`. Review metadata uses the optional free-text `verdict` field for recommendations such as `值得一看`; numeric ratings are not part of the content contract.
